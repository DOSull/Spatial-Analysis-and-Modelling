...
```{r}
pa.counties <- st_read('pa.gpkg')
pa.weather <- st_read('paw19930401.gpkg')
```
...
ts <- SpatialGridDataFrame(
        output, 
        data.frame(var1.pred = predict(ts, newdata = output)))
...
From this plot, we can estimate a range (say around 50) and a sill value (say 30), and we then use these to fit a variogram mode to the data.

```{r}
fit.v <- fit.variogram(v, vgm(psill = 30, model = 'Gau'))
plot(v, fit.v)
```
...
## The assignment
Using methods either from this session (or in last week's) produce interpolated maps of rainfall and maximum and minimum temperatures from the provided data.
...
Note that you could do this using the knitr functionality of the provided *R Markdown* file, but will obviously need to add additional R code to this document, and also tidy things up generally. Please don't just submit a lightly modified version of the file I have provided&mdash;at the very least remove the tutorial material.
